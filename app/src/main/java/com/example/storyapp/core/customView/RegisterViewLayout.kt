...
class RegisterViewLayout : LinearLayout {

    val usernameEditText = CustomUserEditText(context)
    ...
    fun isValidRegister(): Boolean =
        (emailEditText.text ?: "".isNotEmpty()) as Boolean
                && (passwordEditText.text ?: EMPTY_STRING.isNotEmpty()) as Boolean
                && (usernameEditText.text ?: EMPTY_STRING.isNotEmpty()) as Boolean
                && isNoErrorForAllEditText()

    private fun isNoErrorForAllEditText(): Boolean {
        return emailEditText.error == null
